...
function Eye({
  ...
  React.useEffect(() => {
    ...
    let settleFrames = 0
    let lastMouseX = mouseX.current
    let lastMouseY = mouseY.current
    ...
    const update = () => {
      ...
      const cursorMoved = mouseX.current !== lastMouseX || mouseY.current !== lastMouseY
      if (cursorMoved) {
        settleFrames = 40
        lastMouseX = mouseX.current
        lastMouseY = mouseY.current
    ...
    if (eyeRef.current) observer.observe(eyeRef.current)
