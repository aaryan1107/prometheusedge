...
function LogoIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" aria-hidden="true" className={className}>
      <path fill="currentColor" d="M16 2.5 28.5 9v14L16 29.5 3.5 23V9L16 2.5Zm0 4.4L7.5 11.3v9.4l8.5 4.4 8.5-4.4v-9.4L16 6.9Z" />
      <path fill="currentColor" d="M10.5 12.6h11v2.8h-7.8v1.6h6.9v2.6h-6.9v1.8h8v2.8H10.5V12.6Z" />
    </svg>
  );
}
  ...
  return (
    <section className="relative isolate flex min-h-screen w-full overflow-hidden bg-neutral-950 px-1.5 py-1.5 text-white antialiased">
      ...
      >
        ...
        <motion.nav
          variants={navReveal}
          className="relative z-10 mx-auto flex min-h-12 w-full max-w-7xl items-center justify-between px-8 py-3 sm:px-12 lg:px-24"
        >
          <a href="#" className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-white/90">
          ...
          <div className="hidden items-center gap-7 md:flex">
            {navLinks.map((link) => (
              <a key={link.label} href={link.href} className="inline-flex min-h-10 items-center text-sm text-white/80">
                {link.label}
              </a>
            ))}
          </div>

          <a href={signupHref} className="inline-flex min-h-10 items-center justify-center rounded-md bg-zinc-100 px-6 text-sm text-slate-800">
            {signupLabel}
          </a>
        </motion.nav>
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col items-center px-6 pt-16 pb-10 text-center sm:px-12 sm:pt-20 lg:px-24">
          <motion.div variants={softReveal} className="mx-auto max-w-4xl">
            <h1 className="text-[clamp(2.9rem,6.1vw,4.55rem)] leading-[0.98] font-light tracking-normal text-white/95">
          ...
          <motion.p variants={softReveal} className="mt-5 max-w-lg text-sm leading-7 text-white/70">
            {description}
          </motion.p>

          <motion.div variants={softReveal} className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <a href={primaryCtaHref} className="inline-flex min-h-11 items-center justify-center rounded-md bg-zinc-100 px-6 text-sm text-slate-800">
              {primaryCtaLabel}
            </a>
            <a href={secondaryCtaHref} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-white/10 px-5 text-sm text-white">
              {secondaryCtaLabel}
              <ArrowRight className="size-3.5" />
            </a>
          </motion.div>

          <motion.div variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.8 }} className="mt-auto w-full sm:pb-24">
            <motion.p variants={softReveal} className="text-sm text-white/50">
              {trustedEyebrow}
            </motion.p>
            <div className="mx-auto mt-5 flex max-w-lg flex-wrap items-center justify-center gap-x-8 gap-y-4">
              {trustedBrands.map((brand) => (
                <motion.div key={`${brand.mark}-${brand.name}`} variants={brandReveal} className="flex items-center gap-1.5 text-white/40">
                  <TrustedMark mark={brand.mark} />
                  <span className="text-2xl font-semibold tracking-[-0.03em]">{brand.name}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
